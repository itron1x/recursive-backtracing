import at.ac.fhcampuswien.mazeGenerator;
...
public class Main {
    ...
    public void recursive(int x, int y) {
        ...
        for (int i = 0; i < rndmDir.length; i++) {
            switch (rndmDir[i]) {
                case 1: //Up
                    if (x - 2 <= 0) continue;
    ...
    public static void main(String[] args) {
        // Start here
        Main maze = new Main(40);
        maze.recursive(1, 1);
        maze.printMaze();

        mazeGenerator labrinth = new mazeGenerator(5, 5);
        labrinth.generateMaze();
    }
}